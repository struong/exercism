...
class DominoesTest extends AnyFunSuite with Matchers {
  ...
  private def check(input: List[(Int, Int)], hasResult: Boolean): Unit = {
    ...
    if (hasResult) {
      checkChain(
        result getOrElse fail("should have had a chain, but didn't"),
        input
      )
    } else assert(result == None)
  }

  private def checkChain(
      result: List[(Int, Int)],
      input: List[(Int, Int)]
  ): Unit = {
    ...
    def endsShouldMatch: Unit =
      if (!result.isEmpty)
        consecutivesShouldMatch(
          List((result.last, result.length - 1), (result.head, 0))
        )

    assert(
      result.map(sortDomino).sorted == input.map(sortDomino).sorted,
